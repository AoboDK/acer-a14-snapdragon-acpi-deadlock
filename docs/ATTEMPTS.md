...
| 5l | `EFI_MEMORY_ATTRIBUTE_PROTOCOL->ClearMemoryAttributes()` to unprotect DSDT, then patch | 40–41 | **INVALID — wrong GUID** (`{6A7A5CFF...}` = `EFI_COMPONENT_NAME2_PROTOCOL`, not MAP). Correct MAP GUID is `{F4560CF6-40EC-4B4A-A192-BF1D57D0B189}`. MAP never invoked; result does not establish MAP absent. Retest required (D8). |
| 5m | MAP unprotect + canary write to `DSDT[0x20]` (CreatorRevision) + visible stall | 46–47 | **INVALID — same wrong GUID as 5l.** Canary unchanged (`00 00 00 05`), stall observed, but result does not establish MAP absent or ClearMemoryAttributes non-functional. "DSDT write path permanently closed" conclusion withdrawn pending D8 retest. |
